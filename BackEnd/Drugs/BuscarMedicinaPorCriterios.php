...
class Medicina
{
	...
	{
		if (isset($_POST['limiteInferior']) && isset($_POST['cantidad']) && isset($_POST['texto']))
		{
			$host_name = "localhost";
			...
			if ($exists_database)
			{
				...
				$query = "SELECT * FROM Drug WHERE Drugname LIKE '%$texto%' OR Activeingred LIKE '%$texto%' ORDER BY Drugname ASC LIMIT $limite_inferior,$cantidad";
				...
				{					
					...
					$query_total = "SELECT * FROM Drug WHERE Drugname LIKE '%$texto%' OR Activeingred LIKE '%$texto%'";
					$query_resource_total = mysql_query($query_total);
					$cantidad_total =  mysql_num_rows($query_resource_total);
					$cantidad_faltante = $cantidad_total - ($limite_inferior + $cantidad);
